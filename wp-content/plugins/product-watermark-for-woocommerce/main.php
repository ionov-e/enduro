<?php
//Mine-Mine 3 changes _br_backup to _yandex_
    ...
    public function add_watermark_to_images($post_id, $generation = 'create') {
		...
		if( $image_data != FALSE ) {
            ...
            if( $generation == 'restore' ) {
                ...
                $image_name = $image_name . '*_yandex_.' . $image_dimension;
                ...
                if( is_array($files) ) {
                    foreach($files as $backup_filepath) {
                        $filepath = str_replace('_yandex_.', '.', $backup_filepath);
