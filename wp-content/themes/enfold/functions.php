...
// delete sorting from catalog page
remove_action( 'woocommerce_before_shop_loop', 'woocommerce_catalog_ordering', 10 );


// product page: arrows inside category
 ...
 */
 ...
 $avia_config['font_icons'] = apply_filters('avf_default_icons', array(
	...
	'tumblr' 		=> array( 'font' =>'entypo-fontello', 'icon' => 'ue8b7', 'display_name' => 'Telegram' ), //Mine-Mine Telegram icon
...
add_filter( 'big_image_size_threshold', '__return_false' ); //no -scaled images
...
/* 
 function mine_disable_psp($plugins){
      if(strpos($_SERVER['REQUEST_URI'], '/store/') === FALSE && !is_admin() ) {
         $key = array_search( 'premium-seo-pack/plugin.php' , $plugins );
...
function remove_easy_fancybox() {
     remove_action('wp_head', 'premiumseo_head');
}

function custom_page_title( $title ){
    if( is_shop() ){
        $return = 'Магазин Эндуро Краснодар, Купить ZUUM (ZM) мотоциклы';
    }
     $return = $title;
}
...
function change_meta_shop() { 
    if (is_shop()) { 
//remove_action('wp_head', 'description');
//add_filter( 'option_active_plugins', 'mine_disable_psp' );
//add_action('wp_head', 'premiumseo_head', 1);
//add_filter('wp_title', 'custom_page_title', 20);
//add_filter("pre_get_document_title", "my_callback");
?>
<meta name="description" content="Официальный дилер ZUUM. Эндуро Краснодар. ZUUM мотоциклы. Ремонт эндуро. Купить ZUUM. Купить ZUUM Краснодар. Мотоциклы ZM. Найти запчасти для мотоцикла Zuum ZM"/>
<meta name="keywords" content="Официальный дилер ZUUM, Купить Эндуро Краснодар, ZUUM мотоциклы, Ремонт эндуро, Купить ZUUM, Купить ZUUM Краснодар, Мотоциклы ZM"/>
        <?php
        }
    }
add_action('get_header', 'change_meta_shop');
?>